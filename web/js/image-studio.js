...
// ── Studio server-side persistence (via studio API) ──
...
function _imgSrc(d, recordId) {
  ...
  if (d._url) return d._url;
  if (d.file && recordId) return studioMediaUrl("image", recordId, d.file);
  return "";
}

// Helper: ensure base64 is available on an image data object (fetches from server if needed)
async function _ensureImgBase64(d, recordId) {
  if (d.image) return d.image;
  const url = d._url || (d.file && recordId ? studioMediaUrl("image", recordId, d.file) : null);
  if (!url) return null;
  try {
    ...
    d.image = b64; // cache it
    return b64;
  } catch { return null; }
}

// Helper: get base64 from an img element src (handles both data: URLs and http URLs)
async function _imgElBase64(imgEl) {
  if (imgEl.src.startsWith("data:")) {
    return imgEl.src.split(",")[1];
  }
  // Cross-origin safe: draw to canvas
...
// ── Image Folders CRUD ──
async function saveImageFolder(pg) { return _imageAPI.save("folders", pg); }
async function deleteImageFolder(id) { return _imageAPI.remove("folders", id); }
async function loadAllImageFolders() { return _imageAPI.loadAll("folders"); }
let imageFolders = [];
let activeImageFolderId = localStorage.getItem("wooz_image_folder") || null;
async function saveStudioImage(record) { return _imageAPI.save("images", record); }
async function deleteStudioImage(id) { return _imageAPI.remove("images", id); }
async function loadAllStudioImages() { return _imageAPI.loadAll("images"); }

// ── Trash CRUD ──
async function saveToTrash(item) { return _imageAPI.save("trash", item); }
async function loadAllTrash() { return _imageAPI.loadAll("trash"); }
async function deleteFromTrash(id) { return _imageAPI.remove("trash", id); }
async function emptyTrash() { return _imageAPI.clear("trash"); }
function _makeTrashId() {
  return "trash_" + Date.now() + "_" + Math.random().toString(36).slice(2, 6);
}

// ── Favorites CRUD ──
async function saveFavorite(record) { return _imageAPI.save("favorites", record); }
async function deleteFavorite(id) { return _imageAPI.remove("favorites", id); }
async function loadAllFavorites() { return _imageAPI.loadAll("favorites"); }
...
async function refreshFavoritesPanel() {
  try {
    ...
    const favLightboxList = favs.map(f => {
      const src = f.image ? `data:image/png;base64,${f.image}` : (f._imgUrl || "");
      return { src, base64: f.image || null, rawPrompt: f.prompt, body: f.body, model: f.model, width: f.width, height: f.height };
    });

    favs.forEach((fav, favIdx) => {
      const favSrc = fav.image ? `data:image/png;base64,${fav.image}` : (fav._imgUrl || "");
      const card = document.createElement("div");
      card.className = "fav-card";
      card.dataset.favId = fav.id;
      card.innerHTML = `
        <img src="${favSrc}" alt="${esc(fav.prompt)}" draggable="false" />
        <span class="img-res-label"></span>
        <div class="fav-card-actions">
          <button class="action-btn fav-dl" title="Download">${icon("download", 12)}</button>
          <button class="action-btn fav-reuse" title="Reuse settings">${icon("refresh", 12)}</button>
          <button class="action-btn fav-vary" title="Generate variation">${icon("bolt", 12)}</button>
          <button class="action-btn fav-remove img-del is-fav" title="Remove from favorites"><svg width="12" height="12"><use href="#i-heart-filled"/></svg></button>
        </div>
      `;
...
function updateFavCount(count) {
  ...
  if (studioFavCountLabel) { studioFavCountLabel.textContent = count || ""; studioFavCountLabel.style.display = count ? "" : "none"; }
}
    ...
    favs.forEach(fav => {
      ...
      if (parts) {
        ...
        if (parentResult) {
          ...
          if (wrap) {
            const fb = wrap.querySelector(".img-fav");
            if (fb) { fb.classList.add("is-fav"); setFavFilled(fb, true); fb.dataset.favId = fav.id; }
          }
        }
      }
    });
...
// Session-wide image metadata map: key = "recordId_imgIdx" -> metadata object
const _imgMetaMap = new Map();

function _registerImgMeta(recordId, imgIdx, meta) {
  _imgMetaMap.set(`${recordId}_${imgIdx}`, meta);
}

function _clearSessionImgMeta() {
  _imgMetaMap.clear();
}

// Build lightbox list from ALL images currently in the session canvas
function _buildSessionLightboxList(clickedRecordId, clickedImgIdx) {
  const allWraps = studioCanvas.querySelectorAll(".studio-img-wrap");
  const list = [];
  let clickIndex = 0;
  const crid = String(clickedRecordId);
  allWraps.forEach(wrap => {
    const img = wrap.querySelector("img");
    if (!img) return;
    const result = wrap.closest(".studio-result");
    if (!result) return;
    const recordId = result.dataset.studioId;
    const imgIdx = parseInt(wrap.dataset.idx);
    const key = `${recordId}_${imgIdx}`;
    const meta = _imgMetaMap.get(key) || {};
    if (recordId === crid && imgIdx === clickedImgIdx) {
      clickIndex = list.length;
    }
    list.push({ src: img.src, recordId, imgIdx, ...meta });
  });
  return { list, clickIndex };
}
// images: array of { src, base64, recordId, imgIdx, rawPrompt, body }

function openLightbox(src, imageList, currentIndex, opts) {
  if (imageList && imageList.length > 0) {
    _lightboxContext = { images: imageList, index: currentIndex || 0 };
  } else {
    _lightboxContext = { images: [{ src }], index: 0 };
  }
  // Hide studio-specific actions when opened in simple/chat mode
  studioLightbox.classList.toggle("simple-mode", !!(opts && opts.simple));
...
function _renderLightboxImage() {
  ...
  if (favBtn && img.recordId != null) {
    ...
    const _lbIsFav = !!document.querySelector(`.img-fav.is-fav[data-fav-id="${favId}"]`);
    favBtn.classList.toggle("is-fav", _lbIsFav);
    setFavFilled(favBtn, _lbIsFav);
  }
}
...
function closeLightbox() {
  studioLightbox.classList.remove("open", "simple-mode");
...
studioLightbox.querySelector(".lb-fav").addEventListener("click", async e => {
  ...
  if (cardFav) { cardFav.classList.toggle("is-fav", isFav); setFavFilled(cardFav, isFav); cardFav.dataset.favId = favId; }
  refreshFavoritesPanel();
});
...
studioLightbox.querySelector(".lb-del").addEventListener("click", async e => {
  ...
  const img = _lightboxContext.images[_lightboxContext.index];
  if (!img || img.recordId == null) return;
  // Delete image and move to trash
  try {
    const imgB64 = img.base64 || await _imgElBase64(studioLightboxImg);
    await saveToTrash({ id: _makeTrashId(), deletedAt: Date.now(), image: imgB64, rawPrompt: img.rawPrompt, body: img.body, model: img.model, width: img.width, height: img.height });
    _refreshTrashBadge();
    _imgMetaMap.delete(`${img.recordId}_${img.imgIdx}`);
    ...
    if (resultEl) {
      ...
      if (wrap) wrap.remove();
      const remaining = resultEl.querySelectorAll(".studio-img-wrap");
      if (remaining.length === 0) {
        await deleteStudioImage(img.recordId);
        resultEl.remove();
      } else {
        remaining.forEach((w, i) => w.dataset.idx = i);
        const grid = resultEl.querySelector(".studio-result-images");
        if (grid) grid.className = `studio-result-images grid-${remaining.length}`;
      }
    }
  } catch {}
...
function openInpaintEditor(src, base64, recordId, imgIdx, rawPrompt, body) {
  ...
  if (_inpaint._resetModal) _inpaint._resetModal();
...
_inpaint._resetModal = makeModalDraggable(
  document.getElementById("inpaint-card"),
  document.getElementById("inpaint-header")
);
...
_inpaint.resetBtn.addEventListener("click", async () => {
  ...
  _inpaint.unsaved = false;
  _inpaint.saveBtn.classList.remove("has-result");
  // Remove the last saved inpaint record from server and canvas
...
// Restore saved images from server
...
async function restoreStudioImages(forceReload = false) {
  ...
  try {
    ...
    for (const rec of filtered) {
      appendStudioResult(rec.images, rec.rawPrompt, rec.body, rec.id, rec.timestamp);
    }
...
async function renderTrashGrid() {
  ...
  items.forEach(item => {
    ...
    const trashSrc = item.image ? `data:image/png;base64,${item.image}` : (item._imgUrl || "");
    card.innerHTML = `
      <img src="${trashSrc}" alt="${esc(item.rawPrompt || "")}" />
...
async function _restoreFromTrash(item) {
  ...
  // Ensure we have base64 for the image (may need to fetch from server)
  const imgBase64 = item.image || await (async () => {
    if (!item._imgUrl) return null;
    try {
      const res = await fetch(item._imgUrl, { headers: { Authorization: `Bearer ${getToken()}` } });
      const blob = await res.blob();
  ...
  const record = {
    ...
    images: [{ image: imgBase64, model: item.model, width: item.width, height: item.height, elapsed_s: 0 }],
    timestamp: Date.now(),
  };
  await saveStudioImage(record);
...
function renderImageFoldersSidebar() {
  ...
  imageFolders.forEach(pg => {
    ...
    row.addEventListener("drop", async e => {
      ...
      if (!sid) return;
      try {
        // Get all items for this session and move them to the folder
        const res = await apiFetch(`/studio/image/items?session_id=${encodeURIComponent(sid)}`);
        const matching = res.ok ? await res.json() : [];
        for (const rec of matching) {
          await apiFetch(`/studio/image/items/${rec.id}`, {
            method: "PATCH",
            body: JSON.stringify({ folder_id: pg.id }),
          });
        }
  ...
  try {
    ...
    for (const rec of filtered) {
      ...
      if (!sessionMap.has(sid)) {
        sessionMap.set(sid, { session_id: sid, records: [], firstPrompt: rec.title || rec.sessionName || rec.rawPrompt, latestTimestamp: rec.timestamp, imageCount: 0 });
      }
      ...
      // Use title/sessionName if available on any record in the session
      const recName = rec.title || rec.sessionName;
      if (recName && !sess._hasSessionName) {
        sess.firstPrompt = recName;
...
function showGroupedSessionCtxMenu(sess, itemEl, e) {
  ...
  renameItem.addEventListener("click", async () => {
    ...
    try {
      ...
      if (firstRec) {
        await apiFetch(`/studio/image/items/${firstRec.id}`, {
          method: "PATCH",
...
// Advanced toggle
document.getElementById("studio-advanced-toggle").addEventListener("click", function() {
  this.classList.toggle("open");
  document.getElementById("studio-advanced-body").classList.toggle("open");
});
...
studioModelSelect.addEventListener("change", async () => {
  ...
  try {
    ...
    if (res.status === 403) {
      const err = await res.json().catch(() => ({}));
      const m = (err.detail || "").match(/https:\/\/huggingface\.co\/[^\s]+/);
...
studioPrompt.addEventListener("keydown", (e) => {
  if (e.key === "Enter" && !e.shiftKey) {
    e.preventDefault();
    if (studioPrompt.value.trim().startsWith("/")) return; // slash command
    studioGenerate();
  }
});
...
function createStudioPlaceholder() {
  return _createGenProgress("image");
}
...
async function studioGenerate() {
  ...
  // Clear the prompt now that the request is in flight - matches the queue path.
  studioPrompt.value = "";
  ...
  try {
    ...
    if (count === 1) {
      ...
      const liveFooter = liveResult.querySelector(".studio-live-footer");
      if (liveFooter) liveFooter.remove();
      // Populate meta pills now that generation is complete
      const totalElapsed = results.reduce((s, d) => s + (d.elapsed_s || 0), 0);
      const pillsEl = liveResult.querySelector(".meta-pills");
      if (pillsEl) {
        const toggle = pillsEl.querySelector(".prompt-toggle");
        const pills = [];
        if (results[0].model) pills.push(`<span class="meta-pill">${esc(results[0].model)}</span>`);
        if (results[0].width) pills.push(`<span class="meta-pill">${results[0].width}\u00d7${results[0].height}</span>`);
        if (body?.steps) pills.push(`<span class="meta-pill">${body.steps} steps</span>`);
        if (body?.guidance_scale != null) pills.push(`<span class="meta-pill">cfg ${body.guidance_scale}</span>`);
        if (totalElapsed > 0) pills.push(`<span class="meta-pill">${totalElapsed.toFixed(1)}s</span>`);
        if (toggle) toggle.insertAdjacentHTML("beforebegin", pills.join(""));
      }
      // Update the server record with all images
          ...
          if (sessionRecs.length <= 1) {
            ...
            if (nameResp.ok) {
              ...
              if (name && sessionRecs[0]) {
                await apiFetch(`/studio/image/items/${sessionRecs[0].id}`, {
                  method: "PATCH",
...
function _appendImageToGrid(grid, result, data, idx, id, rawPrompt, body, allResults) {
  // Register metadata for session-wide lightbox navigation
  _registerImgMeta(id, idx, { base64: data.image || null, rawPrompt, body, model: data.model, width: data.width, height: data.height });
  ...
  wrap.querySelector(".img-fav").addEventListener("click", async () => {
    ...
    if (isFav) {
      const b64 = await _ensureImgBase64(data, id);
      await saveFavorite({ id: favId, image: b64, model: data.model, width: data.width, height: data.height, prompt: rawPrompt, body, timestamp: Date.now() });
    } else {
      await deleteFavorite(favId);
    }
    refreshFavoritesPanel();
  ...
  wrap.querySelector(".img-del").addEventListener("click", async () => {
    ...
    try {
      const imgIdx = parseInt(wrap.dataset.idx);
      const imgB64 = await _ensureImgBase64(data, id);
      await saveToTrash({ id: _makeTrashId(), deletedAt: Date.now(), image: imgB64, rawPrompt, body, model: data.model, width: data.width, height: data.height, session_id: allResults._sessionId, folder_id: allResults._folderId });
      _imgMetaMap.delete(`${id}_${imgIdx}`);
      _refreshTrashBadge();
      // If no images remain, remove the entire result card
      const remaining = result.querySelectorAll(".studio-img-wrap");
      if (remaining.length === 0) {
        await deleteStudioImage(id);
        result.remove();
      } else {
        // Re-index remaining wraps and update grid
        remaining.forEach((w, i) => w.dataset.idx = i);
        grid.className = `studio-result-images grid-${remaining.length}`;
      }
    } catch {};
  });
  ...
  const pillsEl = result.querySelector(".meta-pills");
  if (pillsEl) {
    pillsEl.innerHTML = `
...
function appendStudioResult(dataArr, rawPrompt, body, recordId, timestamp) {
  ...
  result.innerHTML = `
    ...
    <div class="studio-meta-details">
      <div class="meta-toggles">
        <button class="details-toggle open">${icon("chevron-down", 8)} Details</button>
        <button class="prompt-toggle">${icon("chevron-down", 8)} Prompt</button>
      </div>
      <div class="meta-collapsible meta-pills open">
        ...
        <span class="meta-pill">${formatStudioTimestamp(timestamp || Date.now())}</span>
      </div>
      <div class="meta-prompt">${esc(rawPrompt)}${preset ? ` <span style="color:var(--accent);">[${preset}]</span>` : ""}</div>
    </div>
  `;
  ...
  // Register metadata for session-wide lightbox navigation
  dataArr.forEach((d, i) => {
    _registerImgMeta(id, i, { base64: d.image || null, rawPrompt, body, model: d.model, width: d.width, height: d.height });
  });
  ...
  // Details toggle
  const detailsToggle = result.querySelector(".details-toggle");
  if (detailsToggle) {
    detailsToggle.addEventListener("click", () => {
      detailsToggle.classList.toggle("open");
      result.querySelector(".meta-pills").classList.toggle("open");
    });
  }
  ...
  result.querySelectorAll(".studio-img-wrap").forEach(wrap => {
    ...
    wrap.querySelector(".img-fav").addEventListener("click", async () => {
      ...
      favBtn.dataset.favId = favId;
      if (isFav) {
        const b64 = await _ensureImgBase64(imgData, id);
        await saveFavorite({ id: favId, image: b64, model: imgData.model, width: imgData.width, height: imgData.height, prompt: rawPrompt, body, timestamp: Date.now() });
      } else {
        await deleteFavorite(favId);
      }
      refreshFavoritesPanel();
    ...
    wrap.querySelector(".img-del").addEventListener("click", async () => {
      ...
      try {
        const imgData = dataArr[idx];
        const imgB64 = await _ensureImgBase64(imgData, id);
        await saveToTrash({ id: _makeTrashId(), deletedAt: Date.now(), image: imgB64, rawPrompt, body, model: imgData.model, width: imgData.width, height: imgData.height, session_id: dataArr._sessionId, folder_id: dataArr._folderId });
        _imgMetaMap.delete(`${id}_${idx}`);
        _refreshTrashBadge();
        const remaining = result.querySelectorAll(".studio-img-wrap");
        if (remaining.length === 0) {
          await deleteStudioImage(id);
  ...
  // Persist to server (unless restoring from DB)
  if (!recordId) {
    // Auto-create a session if none active
    if (!activeStudioSessionId) {
      activeStudioSessionId = "sess_" + Date.now() + "_" + Math.random().toString(36).slice(2, 6);
