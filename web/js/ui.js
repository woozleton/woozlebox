...
// ── Prompt modal (text input dialog) ──
function showPromptModal({ title = "Rename", label = "", value = "", okLabel = "Save", placeholder = "" } = {}) {
  return new Promise(resolve => {
    ...
    function onOk() { finish(inp.value.trim() || null); }
...
function _dismissToast() {
  ...
  toastExitTimer = setTimeout(() => { toast.style.display = "none"; toast.classList.remove("toast-exit"); }, 300);
}

function _showToastEl() {
  clearTimeout(toastTimer);
  clearTimeout(toastExitTimer);
  toast.classList.remove("toast-exit");
  toast.style.display = "block";
  // Position horizontally centered over the active prompt box
  const promptBox = document.querySelector(".studio-canvas.active .prompt-box")
  ...
  toastTimer = setTimeout(_dismissToast, 5000);
}
...
function showToast(msg, type) {
  toast.className = type ? "toast-" + type : "";
...
function showGatedRepoToast(repoUrl) {
  toast.className = "toast-error";
  toast.innerHTML = `<span style="display:inline-flex;align-items:center;gap:6px;flex-wrap:wrap;">HuggingFace model requires license acceptance. <a href="${repoUrl}" target="_blank" rel="noopener" style="color:inherit;text-decoration:underline;font-weight:600;">Accept terms here</a></span>`;
  if (toastTimer) clearTimeout(toastTimer);
  toastTimer = setTimeout(_dismissToast, 12000);
  toast.classList.remove("toast-enter");
  void toast.offsetWidth;
  toast.classList.add("toast-enter");
}
...
ctxRename.addEventListener("click", async () => {
  const name = await showPromptModal({ title: "Rename Conversation", label: "Conversation name:" });
  if (!name) return;
...
// Notetaker summary panel resize is called from notetaker.js after its DOM refs are ready
// Code panel resize is called from code-studio.js after its DOM refs are ready

// ── Draggable modal utility ──
// Makes a modal card draggable by its header and optionally corner-resizable.
// Returns a reset() function to call when re-opening the modal.
function makeModalDraggable(cardEl, headerEl, opts = {}) {
  let dragging = false, dragX, dragY, cardX = 0, cardY = 0;
  const persistKey = opts.persistKey || null;
  // Restore persisted position/size so the first open is already in place.
  if (persistKey) {
    try {
      const rawPos = localStorage.getItem(`${persistKey}_pos`);
      if (rawPos) {
        const p = JSON.parse(rawPos);
        if (typeof p.tx === "number" && typeof p.ty === "number") {
          cardX = p.tx; cardY = p.ty;
          cardEl.style.transform = `translate(${cardX}px, ${cardY}px)`;
        }
      }
      const rawSize = localStorage.getItem(`${persistKey}_size`);
      if (rawSize) {
        const s = JSON.parse(rawSize);
        if (typeof s.w === "number" && typeof s.h === "number") {
          cardEl.style.width = s.w + "px";
          cardEl.style.maxWidth = s.w + "px";
          cardEl.style.height = s.h + "px";
          cardEl.style.maxHeight = s.h + "px";
        }
      }
    } catch {}
  }
  ...
  headerEl.addEventListener("mousedown", (e) => {
    ...
    dragging = true;
    dragX = e.clientX - cardX;
    dragY = e.clientY - cardY;
  ...
  window.addEventListener("mousemove", (e) => {
    if (!dragging) return;
    cardX = e.clientX - dragX;
    cardY = e.clientY - dragY;
    cardEl.style.transform = `translate(${cardX}px, ${cardY}px)`;
  });
  window.addEventListener("mouseup", () => {
    ...
    if (persistKey) {
      try {
        localStorage.setItem(`${persistKey}_pos`, JSON.stringify({ tx: cardX, ty: cardY }));
  ...
  const resizeHandle = opts.resizeHandle || cardEl.querySelector(".modal-resize-handle");
  if (resizeHandle) {
    ...
    resizeHandle.addEventListener("mousedown", (e) => {
      ...
      rStartX = e.clientX;
      rStartY = e.clientY;
      rStartW = cardEl.offsetWidth;
      rStartH = cardEl.offsetHeight;
    ...
    window.addEventListener("mousemove", (e) => {
      ...
      const newW = Math.max(minW, rStartW + (e.clientX - rStartX));
      cardEl.style.width = newW + "px";
      cardEl.style.maxWidth = newW + "px";
      const newH = Math.max(minH, rStartH + (e.clientY - rStartY));
      cardEl.style.maxHeight = newH + "px";
    ...
    window.addEventListener("mouseup", () => {
      ...
      if (persistKey) {
        try {
          localStorage.setItem(`${persistKey}_size`, JSON.stringify({
  ...
  return function reset() {
    ...
    if (persistKey) {
      try {
        localStorage.removeItem(`${persistKey}_pos`);
        localStorage.removeItem(`${persistKey}_size`);
        if (typeof scheduleSettingsSync === "function") scheduleSettingsSync();
      } catch {}
    }
  };
...
// ── Wire draggable modals ──
// Settings dialog
(function() {
  const card = document.getElementById("settings-dialog");
  const header = document.getElementById("settings-header");
  if (card && header) {
    const resetSettings = makeModalDraggable(card, header, {
      resizeHandle: document.getElementById("settings-resize"),
      minWidth: 480,
      minHeight: 360,
    });
    // Reset position each time settings opens
    const panel = document.getElementById("settings-panel");
    const obs = new MutationObserver(() => { if (panel.classList.contains("open")) resetSettings(); });
...
// Trash modal
(function() {
  const inner = document.querySelector("#shared-trash-modal .popout-modal-inner");
  const header = document.querySelector("#shared-trash-modal .popout-modal-header");
  if (inner && header) makeModalDraggable(inner, header);
})();
