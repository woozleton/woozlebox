...
async function loadAdminUsers() {
  ...
  try {
    ...
    users.forEach(u => {
      ...
      if (!u.is_active) card.classList.add("inactive");
      card.innerHTML = `
        <div class="admin-avatar">${adminAvatarInner}</div>
        <div class="admin-user-name-col">
          <span class="admin-user-name">${esc(u.username)}</span>
          ${profileName ? `<span class="admin-user-profile-name">${esc(profileName)}</span>` : ""}
        </div>
        <button class="role-badge ${u.role} ${u.id === currentUser.id ? "role-badge-self" : "role-badge-btn"}" data-uid="${u.id}" ${u.id === currentUser.id ? "disabled title='Cannot change your own role'" : "title='Click to toggle role'"}>${u.role}</button>
        ...
        <div class="admin-user-actions">
          ...
          ${u.id !== currentUser.id
            ? `<button class="admin-icon-btn danger admin-del-btn" data-uid="${u.id}" data-uname="${esc(u.username)}" title="Delete user">${icon("trash-lines", 15)}</button>`
            : `<button class="admin-icon-btn admin-del-btn-disabled" disabled title="Cannot delete your own account">${icon("trash-lines", 15)}</button>`}
        </div>
      `;
    ...
    list.querySelectorAll(".role-badge-btn").forEach(btn => {
      btn.addEventListener("click", async () => {
        const newRole = btn.classList.contains("admin") ? "user" : "admin";
        const r = await apiFetch(`/admin/users/${btn.dataset.uid}`, {
          method: "PATCH",
          body: JSON.stringify({ role: newRole }),
        });
        if (r?.ok) {
          btn.classList.remove("admin", "user");
          btn.classList.add(newRole);
...
function loadAdminDefaults() {
  ...
  const srcSel = document.getElementById("chat-model-select");
