...
// ── Video server-side persistence (via studio API) ──
const _videoAPI = createStudioAPI("video");

async function saveVideoClip(record)    { return _videoAPI.save("videos", record); }
async function loadAllVideoClips()      { return _videoAPI.loadAll("videos"); }
async function deleteVideoClip(id)      { return _videoAPI.remove("videos", id); }
async function saveVideoFavorite(rec)   { return _videoAPI.save("favorites", rec); }
async function deleteVideoFavorite(id)  { return _videoAPI.remove("favorites", id); }
async function loadAllVideoFavorites()  { return _videoAPI.loadAll("favorites"); }
async function isVideoFavorite(id)      { return _videoAPI.has("favorites", id); }
async function loadAllVideoTrash()      { return _videoAPI.loadAll("trash"); }
async function deleteFromVideoTrash(id) { return _videoAPI.remove("trash", id); }
async function emptyVideoTrash()        { return _videoAPI.clear("trash"); }
async function saveVideoFolder(col)     { return _videoAPI.save("folders", col); }
async function deleteVideoFolder(id)    { return _videoAPI.remove("folders", id); }
async function loadAllVideoFolders()    { return _videoAPI.loadAll("folders"); }

// Helper: get video playback URL (blob from base64, or server URL)
function _videoPlayUrl(record) {
  if (record.video) {
    const byteStr = atob(record.video);
    const bytes = new Uint8Array(byteStr.length);
    for (let i = 0; i < byteStr.length; i++) bytes[i] = byteStr.charCodeAt(i);
    return URL.createObjectURL(new Blob([bytes], { type: "video/mp4" }));
  }
  return record._videoUrl || studioMediaUrl("video", record.id, "video.mp4");
}
...
async function renderVideoSessionsList() {
  ...
  for (const s of sessions) {
    ...
    row.className = "sb-item video-session-item" + (s.id === activeVideoSessionId ? " active" : "");
    row.dataset.sessionId = s.id;
    row.draggable = true;
    const label = s.title || s.clips[0]?.rawPrompt?.slice(0, 40) || "Untitled";
    const count = s.clips.length;
    row.innerHTML = `<span class="sb-item-title">${esc(label)}</span><span class="sb-item-badge">${count}</span><button class="sb-item-menu video-session-menu" title="Options">&#x22EF;</button>`;
    row.addEventListener("dragstart", e => {
      e.dataTransfer.setData("text/video-session", s.id);
      row.classList.add("dragging");
    });
    row.addEventListener("dragend", () => row.classList.remove("dragging"));
    row.addEventListener("click", e => {
      if (e.target.closest(".video-session-menu")) return;
      activeVideoSessionId = s.id;
      localStorage.setItem("wooz_video_session", s.id);
      restoreVideoClips();
      renderVideoSessionsList();
    });
    row.querySelector(".video-session-menu").addEventListener("click", e => {
      e.stopPropagation();
      _showVideoSessionMenu(s, row, e);
    });
    list.appendChild(row);
  }
}

function _showVideoSessionMenu(sess, itemEl, e) {
  document.querySelectorAll(".video-session-ctx-menu, .video-folder-sub-menu").forEach(m => m.remove());
  const menu = document.createElement("div");
  menu.className = "video-session-ctx-menu";
  ...
  renameItem.addEventListener("click", async () => {
    menu.remove();
    const name = await showPromptModal({ title: "Rename Session", label: "Session name:", value: sess.title || sess.clips[0]?.rawPrompt?.slice(0, 40) || "Untitled" });
    if (!name) return;
    try {
      const firstClip = sess.clips[sess.clips.length - 1];
      if (firstClip) {
        await apiFetch(`/studio/video/items/${firstClip.id}`, {
          method: "PATCH",
  ...
  if (videoFolders.length > 1) {
    ...
    moveBtn.addEventListener("mouseenter", () => {
      ...
      document.querySelectorAll(".video-folder-sub-menu").forEach(m => m.remove());
      const sub = document.createElement("div");
      sub.className = "video-folder-sub-menu";
      ...
      videoFolders.filter(f => f.id !== activeVideoFolderId).forEach(folder => {
        ...
        item.addEventListener("click", async () => {
          ...
          try {
            for (const c of sess.clips) {
              await apiFetch(`/studio/video/items/${c.id}`, {
                method: "PATCH",
  ...
  deleteItem.addEventListener("click", async () => {
    menu.remove();
    for (const c of sess.clips) {
      await apiFetch(`/studio/video/items/${c.id}/trash`, { method: "POST" });
    }
    _refreshVideoTrashBadge();
    if (activeVideoSessionId === sess.id) {
      activeVideoSessionId = _newVideoSessionId();
      localStorage.setItem("wooz_video_session", activeVideoSessionId);
    }
    renderVideoSessionsList();
  ...
  const _closeMenu = (ev) => {
    if (!menu.contains(ev.target) && !document.querySelector(".video-folder-sub-menu")?.contains(ev.target)) {
      menu.remove(); document.querySelectorAll(".video-folder-sub-menu").forEach(m => m.remove());
    ...
    row.addEventListener("drop", async e => {
      ...
      try {
        ...
        for (const rec of matching) {
          await apiFetch(`/studio/video/items/${rec.id}`, {
            method: "PATCH",
...
function showVideoFolderCtxMenu(col, e) {
  ...
  menu.querySelector('[data-action="rename"]').addEventListener("click", () => {
    menu.remove();
    openVideoFolderModal(col);
  });
  menu.querySelector('[data-action="delete"]').addEventListener("click", async () => {
    menu.remove();
    if (videoFolders.length <= 1) { showToast("At least one folder must exist."); return; }
    const confirmed = await showConfirm({ title: "Delete Folder", message: `Delete "${col.name}"? All videos will be moved to trash.` });
    if (!confirmed) return;
    const allClips = await loadAllVideoClips();
    const toDelete = allClips.filter(r => r.folder_id === col.id);
    for (const clip of toDelete) {
      await apiFetch(`/studio/video/items/${clip.id}/trash`, { method: "POST" });
    }
    _refreshVideoTrashBadge();
...
// Video Folder modal
let _editingVideoFolderId = null;
function openVideoFolderModal(col) {
  _editingVideoFolderId = col ? col.id : null;
  document.getElementById("shared-folder-title").textContent = col ? "Edit Folder" : "New Folder";
  document.getElementById("shared-folder-hint").textContent = "Folders organize your generated videos into separate groups.";
  document.getElementById("shared-folder-name").value = col?.name || "";
  document.getElementById("shared-folder-name").placeholder = "e.g. Animations, Tutorials, Music Videos";
  ...
  document.getElementById("shared-folder-save").onclick = async () => {
    ...
    if (_editingVideoFolderId) {
      const existing = videoFolders.find(c => c.id === _editingVideoFolderId);
      if (existing) { existing.name = name; existing.description = description; await saveVideoFolder(existing); }
    } else {
      const col = { id: "vfolder_" + Date.now(), name, description, timestamp: Date.now() };
...
function closeVideoFolderModal() {
  document.getElementById("shared-folder-modal").classList.remove("open");
  _editingVideoFolderId = null;
}
document.getElementById("video-folder-new-btn").addEventListener("click", () => openVideoFolderModal(null));
...
function createVideoResultCard(record) {
  ...
  // Video playback URL (base64 blob or server URL)
  const blobUrl = _videoPlayUrl(record);
  ...
  el.innerHTML = `
    ...
    <div class="video-result-body">
      ...
      <div class="video-result-actions">
        <button class="action-btn video-fav-btn" title="Favorite">
          ${icon("heart")}
        </button>
        <button class="action-btn video-reuse-btn" title="Reuse settings">
          ${icon("refresh", 12)}
        </button>
        <button class="action-btn video-download-btn" title="Download">
          ${icon("download", 12)}
        </button>
        <button class="action-btn video-delete-btn" title="Delete">
  ...
  isVideoFavorite(record.id).then(fav => {
    if (fav) { favBtn.classList.add("is-fav"); setFavFilled(favBtn, true); }
  });
  favBtn.addEventListener("click", async () => {
    const isFav = favBtn.classList.contains("is-fav");
    if (isFav) {
      await apiFetch(`/studio/video/items/${record.id}/favorite`, { method: "DELETE" });
      favBtn.classList.remove("is-fav");
      setFavFilled(favBtn, false);
    } else {
      await apiFetch(`/studio/video/items/${record.id}/favorite`, { method: "POST" });
  ...
  el.querySelector(".video-delete-btn").addEventListener("click", async () => {
    await apiFetch(`/studio/video/items/${record.id}/trash`, { method: "POST" });
    el.remove();
...
async function videoGenerate() {
  ...
  const progress = _createGenProgress("video");
  videoCanvasEmpty.style.display = "none";
...
videoPrompt.addEventListener("keydown", (e) => {
  if (e.key === "Enter" && !e.shiftKey) {
    e.preventDefault();
    if (videoPrompt.value.trim().startsWith("/")) return; // slash command
    videoGenerate();
  }
});
...
function _createVideoFavCard(fav) {
  ...
  card.innerHTML = `
    ...
    <div class="music-fav-card-actions">
      <button class="action-btn video-fav-dl" title="Download">${icon("download", 12)}</button>
      <button class="action-btn video-fav-reuse" title="Reuse settings">${icon("refresh", 12)}</button>
      <button class="action-btn video-fav-remove is-fav" title="Remove from favorites"><svg width="12" height="12"><use href="#i-heart-filled"/></svg></button>
    </div>
  `;
...
async function _updateVideoFavBadge() {
  ...
  if (videoFavCountLabel) { videoFavCountLabel.textContent = count || ""; videoFavCountLabel.style.display = count ? "" : "none"; }
}
...
async function renderVideoTrashList() {
  ...
  items.forEach(item => {
    ...
    card.querySelector(".studio-trash-restore").addEventListener("click", async () => {
      await apiFetch(`/studio/video/items/${item.id}/restore`, { method: "POST" });
      if ((item.folder_id || null) === (activeVideoFolderId || null)) {
        videoCanvas.appendChild(createVideoResultCard(item));
        videoCanvasEmpty.style.display = "none";
        renderVideoSessionsList();
      }
      await renderVideoTrashList();
