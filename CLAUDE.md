...
- Image Studio has its own incognito toggle (`window.imageIncognito` in `image-studio.js`, mask-icon): generated images are shown and downloadable but never persisted - `_persistStudioRecord` and the other `saveStudioImage` auto-save paths are gated on the flag (image-api is stateless, so gating the frontend is sufficient). A `body.image-incognito-active` class draws the same alert border around `.studio-main-area`.
